...
import { LayoutGrid, FileText, Image as ImageIcon, Palette, Clock } from 'lucide-react';

export default function Sidebar({ activeTab, setActiveTab }) {
  const menuItems = [
    { id: 'studio', label: 'Content Studio', icon: ImageIcon },
    { id: 'briefs', label: 'Brief Creator', icon: FileText },
    ...
    <aside className="sidebar-container">
      ...
      <div style={{ marginTop: 'auto', padding: '18px', borderTop: '1px solid var(--border-subtle)' }}>
        <div style={{ background: 'rgba(0,0,0,0.3)', padding: '12px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }}>
          <p style={{ fontSize: '0.74rem', fontWeight: 700, color: 'var(--text-main)' }}>AI Mode: Mock Provider</p>
          <p style={{ fontSize: '0.68rem', color: 'var(--text-dim)', marginTop: '2px' }}>Zero Cost • Deterministic Test Ready</p>
        </div>
      </div>
    </aside>
  );
